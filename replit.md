...
## Регистрация студентов (обновлено)

Регистрация теперь работает через очередь заявок:
1. Студент заполняет **обязательные поля ФИО** (Фамилия, Имя) + Отчество (необязательно) + логин/пароль/email
2. Email-верификация через 4-значный код
3. Создаётся запись в `pending_registrations` со статусом `pending` — аккаунт НЕ создаётся сразу
4. Администратор видит все заявки во вкладке **«Заявки на регистрацию»** в AdminPanel
5. Админ сверяет ФИО с загруженным CSV-реестром (вкладка «Реестр») и нажимает **Одобрить** / **Отклонить**
6. При одобрении автоматически создаётся аккаунт студента
7. **Лимит**: не более 3 заявок с одного IP-адреса за 24 часа

## Безопасность

- **Параметризованные SQL-запросы** — защита от SQL-инъекций
- **Санитизация входящих строк** — убирает `< > ' " ; \ \``
- **Rate Limiting** — 10 попыток входа в минуту, 10 запросов регистрации, 5 SMS-кодов
- **IP Rate Limit для регистрации** — макс 3 заявки с одного IP за 24 часа (независимый счётчик)
...
Активная БД: `/home/runner/workspace/kgusport.db` (корень проекта, не в server/).

## Исправленные баги

### Раунд 1
- `PUT /api/profile/:id` возвращал хеш пароля — убрано из ответа
- `DELETE /api/admin/users/:id` падал с FK constraint error — исправлено каскадным удалением
- `DELETE /api/sections/:id` падал с FK constraint error — исправлено каскадным удалением

### Раунд 2
- `POST /api/bookings` принимал пустой body — добавлена валидация
- Формат даты в `Section.jsx` / `BookingConfirm.jsx` был locale-specific — заменён на ISO YYYY-MM-DD
- `BookingConfirm.jsx` не показывал ошибки сервера при неудачном бронировании — исправлено

### Раунд 3
- `POST /api/bookings` — нет защиты от дублирующихся бронирований — добавлена проверка
- `POST /api/bookings` — нет проверки лимита мест — добавлена проверка `max_students`
- `POST /api/bookings` — нет валидации существования секции — добавлена проверка
- `Section.jsx` — ошибка сервера (дубликат/нет мест) не показывалась пользователю — исправлено
- `BookingConfirm.jsx` — аналогичная проблема с ошибками сервера — исправлено
